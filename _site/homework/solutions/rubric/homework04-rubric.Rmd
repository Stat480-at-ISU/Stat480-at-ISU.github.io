...
:::sol
```{r}
bikes$workingday <- !((bikes$holiday) | (bikes$weekday %in% c(1,7))) # (A or B)^c
bikes$workingday <- !(bikes$holiday) & !(bikes$weekday %in% c(1,7)) # A^c and B^c

ggplot(bikes, aes(x = casual, y = registered, color = workingday)) + 
  geom_point()
```

Example interpretation: From this scatterplot we can see that there, in general, appears to be a larger number of registered riders renting bikes than casual riders. This is especially true on work days.
:::

:::rubric
+2.5 pt  
+1 pt for correctly creating the logical variable `workingday`
+1 pt for plot
...
Example interpretation: The median number of of casual rentals is larger is the summer months (months ~5-9) than in the winter months. Additionally, from the boxplots, we can see that the distributions of number of casual rentals tend to be skewed with some larger outliers. 
:::
